...
import { ILiterals, UIISimpleTableDefaultColumnDefs } from '../models/ui-ism-table.model';
 ...
 */
@Component({
	tag: 'scib-ui-ism-table',
	styleUrl: 'ui-ism-table.scss',
	shadow: false,
	scoped: false
})
export class UIIsmTable {
	...
	//@Prop({mutable: true, reflect: true}) columnDefs:

	/** Rendering element for the table */
	@State() _table: HTMLElement;

	@State() _defaultColumns: UIISimpleTableDefaultColumnDefs[] = [{ flex: 1 }];

	@State() _gridOptions: any = {
		masterDetail: true,
		detailCellRenderer: 'myDetailCellRenderer',
		detailCellRendererParams: {},
		//TODO: ROW DATA EN PROP
		rowData: [
			{
				name: 'Nora Thomas',
				account: 177000,
		...
		components: {
			myDetailCellRenderer: Renderer,
			getRowStyle: (params) => {
				const status = params.data.state;
				switch (status) {
					case 'Pending':
					case 'In progress':
	...
	onFirstDataRendered(params) {
		return params.api.forEachNode(function (node) {
	...
	componentDidLoad() {
		this._table = document.getElementById('simple-table');
		this._gridOptions.columnDefs = [
			{ field: 'name' },
			{ field: 'account' },
			{ field: 'calls' },
			{ field: 'minutes', valueFormatter: "x.toLocaleString() + 'm'", cellRenderer: 'agGroupCellRenderer' }
		];
		...
		this._gridOptions.fullWidthCellRenderer = () => {
			let div = document.createElement('div');
			div.innerHTML = '<h1>render component</h1>';
			return div;
		};
		// this._gridOptions.detailCellRenderer = 'myDetailCellRenderer',
		// 	this._gridOptions.onFirstDataRendered = (params) => {
		// 		return this.onFirstDataRendered(params)
		//   }
		new Grid(this._table, this._gridOptions);
	}
		...
		return (
			<Host>
				<div class="ui-simple-table">
					<div id="simple-table" class="ag-theme-material ui-simple-table__table"></div>
				</div>
			</Host>
		);
	}
