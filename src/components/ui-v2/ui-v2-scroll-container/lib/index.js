...
export class ScrollableComponentElement extends HTMLElement {
		...
		for (let orientation of orientations) {
				...
				(event) => {
					...
					this.viewport.scrollTo({
						[spacings[orientation]]:
							(event[clients[orientation]] -
								this.boundingBoxes[orientation].scrollbarTrack[spacings[orientation]] -
								this.sizes[orientation].scrollbarThumb / 2) *
			...
			this.elements[orientation].scrollbarThumb.addEventListener(
				...
				(event) => {
					...
					for (let orientation of orientations) {
						this.scrollingWithThumbOrigin[pages[orientation]] = event.touches
							? event.touches[0][pages[orientation]]
							: event[pages[orientation]];
				...
				for (let orientation of orientations) {
					if (this.isScrollingWithThumb[orientation]) {
						const scrollbarThumbOffset =
							(event.touches ? event.touches[0][pages[orientation]] : event[pages[orientation]]) -
							this.scrollingWithThumbOrigin[pages[orientation]];
						this.viewport[scrollSpacings[orientation]] =
							this.scrollingWithThumbOrigin[scrollSpacings[orientation]] + scrollbarThumbOffset * this.ratios[orientation];
						break;
					}
				}
			},
	...
	updateScrollbarThumbPositions() {
		for (let orientation of orientations) {
			...
			this.elements[
				orientation
			].scrollbarThumb.style.transform = `translate3D(${scrollbarThumbOffset.horizontal}px, ${scrollbarThumbOffset.vertical}px, 0)`;
		}
	}
}
