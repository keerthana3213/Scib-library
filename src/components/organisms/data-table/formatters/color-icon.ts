...
import { cellIconStyle, emitCustomEvent, getTableId, setTooltip } from './common-utils';
...
export const colorIcon = (cell: TableCell, formatterParams) => {
	...
	if (colorIconConfig) {
		...
		if (!isHidden) {
			...
			if (hasTooltip) {
				const tooltipText = get(colorIconConfig, 'tooltipText');
				setTooltip(_tableId, cell, formatterParams, tooltipText);
			}
		}
	}
