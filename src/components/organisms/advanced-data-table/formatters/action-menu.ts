...
export const actionMenu = (cell: TableCell, formatterParams) => {
	...
	iconElement.onclick = () => {
		const { top, right, height, topOffset } = getPosition(_tableId, cell.getElement());
		const topGap = 8;
		...
		const tablescroll = References.instance.getHostRef(_tableId).shadowRoot.querySelector('.scroll-tableholder');
		menu.setAttribute('item-list', JSON.stringify(compact(itemList)));
		menu.style.setProperty('right', `${right}px`);
		menu.style.setProperty(
			'top',
			`${tablescroll ? top + topOffset - tablescroll.scrollTop + cell.getElement().offsetHeight : top + height + (topOffset - topGap)}px`
		);
		menu.openMenu();
	};
	return iconElement;
