...
import { AppsEcmvFormAddlFiles } from '../component/ecmv-form-add-files';
...
describe('apps-ecmv-form-add-files', () => {
	// const metadataValues = ;
	it('builds clean', () => {
		expect(new AppsEcmvFormAddlFiles()).toBeTruthy();
	});

	it('test readMode', () => {
		const ecmvFormAddFiles = new AppsEcmvFormAddlFiles();
		expect(ecmvFormAddFiles.readMode).toEqual(false);
		ecmvFormAddFiles.setReadMode(true);
		expect(ecmvFormAddFiles._readMode).toEqual(true);
	});

	xit('test getECMViewerPermissions write permission', async () => {
		const page = await newSpecPage({
			components: [AppsEcmvFormAddlFiles],
			template: () => <scib-apps-ecmv-form-add-files></scib-apps-ecmv-form-add-files>
		});
		let eventSpy = jest.fn();
		page.win.addEventListener('showPermissionsRequired', eventSpy);
		// EcmViewerService
		// (EcmViewerService.getFSEECMVPermissions as any).mockImplementation(() => {return new Promise((r,rr) => {return r(ECMPermision)})})
		jest.spyOn(EcmViewerService, 'getFSEECMVPermissions').mockImplementation(() => {
			return new Promise((r) => {
				return r(ECMPermision);
			});
		});
		const getFSEPermissionsDataSpy = jest.spyOn(EcmViewerAddFormFilesService, 'getFSEPermissionsData');
		const getAllDocumentsSpy = jest.spyOn(EcmViewerAddFormFilesService, 'getAllDocuments');

		expect(eventSpy).not.toHaveBeenCalled();
		expect(getFSEPermissionsDataSpy).toHaveBeenCalled();
		expect(getAllDocumentsSpy).toHaveBeenCalled();
	});

	xit('test getECMViewerPermissions read permission', async () => {
		const page = await newSpecPage({
			components: [AppsEcmvFormAddlFiles],
			template: () => <scib-apps-ecmv-form-add-files></scib-apps-ecmv-form-add-files>
		});
		let eventSpy = jest.fn();
		page.win.addEventListener('showPermissionsRequired', eventSpy);
		// (EcmViewerService.getFSEECMVPermissions as any).mockImplementation(() => { return new Promise((r,rr) => {return r(ECMPermisionOnlyRead)})})
		// jest.spyOn(EcmViewerService, 'getFSEECMVPermissions').mockImplementation(() => {
		// 	return new Promise((r) => {
		// 		return r(ECMPermisionOnlyRead);
		// 	});
		// });
	...
	it('should put multiselect attribute to true in DocumentOwner metadata value when execute watchMetadataValues() function', () => {
		const component = new AppsEcmvFormAddlFiles();
		component.itemsMetadata = metadataValues;
		component._multiChoiceMetadata = ['ProductLevel1'];
		component.watchMetadataValues();
		expect(component.selectItem.find((item) => item.id_doc_metadata === 'ProductLevel1')?.multiselect).toEqual(true);
	});

	it('should save correctly in formDataMap variable the selected metadata values for DocumentOwner', () => {
		let event = {
			detail: [
				{
					label: 'Nda',
		...
		const expectedResponse = ['Nda', 'LetterOfCredit'];
		const component = new AppsEcmvFormAddlFiles();
		component.itemsMetadata = metadataValues;
		component.watchMetadataValues();
		component.fillFormDataMapSelect(event, 'ProductLevel1', 'multi');
		expect(component.formDataMap['ProductLevel1']).toEqual(expectedResponse);
	});
});
